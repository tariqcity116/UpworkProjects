...
// invariant: no duplicates in this movie collection and Number >= 0
public interface IMovieCollection
{
	...
	//				   This movie collection remains unchanged and new Number = old Number.
	bool IsEmpty();


	// Insert a movie into this movie collection.
	// Pre-condition: nil
	// Post-condition: if the movie was not in this movie collection, the movie has been added into this movie collection,
	//				   new Number = old Number + 1 and return true; otherwise, new Number = old Number and return false.
	bool Insert(IMovie movie);


	// Delete a movie from this movie collection.
	// Pre-condition: nil
	// Post-condition: if the movie was in this movie collection, the movie has been removed out of this movie collection,
	//				   new Number - old Number - 1 and return true; otherwise, return false and this movie collection
	//				   remains unchanged and new Number = old Number.
	bool Delete(IMovie movie);


    // Search for a movie by its title in this movie collection.
    // Pre-condition: nil
    // Post-condition: return the reference of the movie object if the movie is in this movie collection;
    ...
    // Return an array that contains all movies in this movie collection and the movies in the array
	// are sorted in the dictionary order by movie title.
    // Pre-condition: nil
